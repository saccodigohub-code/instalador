...
verificar_versao_pro() {
  ...
  # Verificar se a URL já contém multiflow
  if grep -q "multiflow" "$GIT_CONFIG_FILE"; then
    printf "${YELLOW}══════════════════════════════════════════════════════════════════${WHITE}\n"
    printf "${GREEN}✅ A versão PRO já está configurada!${WHITE}\n"
    echo
    printf "${WHITE}   O repositório já está apontando para ${BLUE}multiflow${WHITE}.\n"
...
atualizar_git_config() {
  ...
  
  {
    ...
    REPO_URL="https://${TOKEN_AUTH}@github.com/scriptswhitelabel/multiflow.git"
    ...
    if grep -Fq "${TOKEN_ANTIGO}@github.com/scriptswhitelabel/multiflow" "$GIT_CONFIG_FILE"; then
      ...
      sed -i "s|url = https://${TOKEN_ANTIGO_ESCAPED}@github.com/scriptswhitelabel/multiflow|url = https://${TOKEN_AUTH}@github.com/scriptswhitelabel/multiflow|g" "$GIT_CONFIG_FILE"
      ...
      if grep -q "url = https://.*@github.com/scriptswhitelabel/multiflow" "$GIT_CONFIG_FILE"; then
        sed -i "s|url = https://.*@github.com/scriptswhitelabel/multiflow|url = https://${TOKEN_AUTH}@github.com/scriptswhitelabel/multiflow|g" "$GIT_CONFIG_FILE"
...
QUEUE_ID="15"
USER_ID=""
MENSAGEM="🚨 INICIANDO Atualização do ${nome_titulo} para multiflow"
...
QUEUE_ID="15"
USER_ID=""
MENSAGEM="🚨 Atualização do ${nome_titulo} FINALIZADA para multiflow"
